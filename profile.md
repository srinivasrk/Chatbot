# About Srinivas Kulkarni

**Name:** Srinivas Kulkarni
**Role:** Software Development Engineer @ AWS
**Location:** Greater Seattle Area
**Email:** srinivasrk92@gmail.com
**GitHub:** https://github.com/srinivasrk
**LinkedIn:** https://www.linkedin.com/in/srini-cincy

I'm an AI-native engineer with hands-on experience building applications using agentic frameworks (OpenAI SDK, CrewAI, LangGraph, AutoGen, and similar). I build robust, end‑to‑end systems that bridge software architecture and cloud-scale infrastructure.

## Highlights

- AWS Supply Chain Optimization — software powering datacenter infrastructure expansion
- AI-native engineering — agentic applications (OpenAI SDK, CrewAI, LangGraph, AutoGen, etc.)
- Cloud-native development with Docker & Kubernetes (CKAD)
- Languages: Java, JavaScript/TypeScript, Python

## Work Experience

### Software Development Engineer — Amazon Web Services (AWS)

**Period:** May 2022 — Present  
**Location:** Seattle, WA

### Software Engineer — Xylem Inc.

**Period:** Aug 2018 — May 2022  
**Location:** Cincinnati Area, KY

### Sr. Analyst — Dell EMC

**Period:** Feb 2015 — Jul 2017  
**Location:** Bangalore

## Skills

**Languages & scripting:** JavaScript, TypeScript, Java, Python, SQL, Linux / Bash
**Frameworks & UI:** React, Next.js, Node.js, Svelte, Tailwind CSS, HTML5, CSS / SCSS
**AI & agents:** OpenAI SDK, CrewAI
**Cloud & DevOps:** AWS, Docker, Kubernetes, Terraform, CI/CD
**Databases & data stores:** DynamoDB, MongoDB, InfluxDB, RDS
**Testing & tooling:** Jest, Git

## Projects

### Stock analyst — CrewAI

**Year:** 2025
**Domain:** AI

Agentic sector research with CrewAI: a multi-agent pipeline uses web search (Serper) and Gemini to produce structured research output. FastAPI backend with async jobs and WebSockets, plus a TypeScript SPA; Auth0 for sign-in.

**Tags:** CrewAI, FastAPI, TypeScript, Gemini, Auth0
**Demo:** https://stocks.srini.fyi
**Repo:** https://github.com/srinivasrk/Market-Researcher---Crew-AI

### Portfolio profile chatbot

**Year:** 2025
**Domain:** AI

A Gradio chat UI on FastAPI that answers in my voice using Google Gemini, grounded in my profile PDF. A scope gate keeps questions on-topic before the main model runs; rate limits help control API cost. Packaged with Docker and embeddable on Hugging Face Spaces.

**Tags:** Python, FastAPI, Gradio, Gemini, Docker
**Repo:** https://github.com/srinivasrk/Chatbot

### Agentic AI design patterns — LangGraph & OpenAI SDK

**Year:** 2025
**Domain:** AI

A hands-on pattern library for multi-agent systems built with LangGraph and the OpenAI SDK. Covers evaluator-optimizer loops (cross-benchmarking Gemini and Ollama), structured tool-calling, and a three-agent moderated debate graph where two adversarial agents argue a position while a moderator orchestrates turn-taking, enforces rebuttal fairness, and drives the graph to a typed verdict via Pydantic-validated state. Patterns use LangGraph's Command routing, annotated reducers, and structured outputs to show how real agentic workflows are wired together.

**Tags:** LangGraph, OpenAI SDK, Python, Gemini, Pydantic
**Repo:** https://github.com/srinivasrk/AgenticAIWorkflows

### Seattle traffic map — PostGIS & Mapbox

**Year:** 2023
**Domain:** Maps & GIS

Animated road congestion on a Mapbox map: centerlines and a full time series per road live in PostgreSQL (PostGIS + TimescaleDB). The browser loads dual vector tile layers (static roads + traffic overlay with embedded frame properties c0, c1, …) and scrubs time without changing tile URLs. FastAPI serves MVT; Next.js 15 client.

**Tags:** PostGIS, TimescaleDB, Mapbox, Next.js, FastAPI
**Repo:** https://github.com/srinivasrk/postgis-mapbox

## Reading List

### 2026

- *American Moonshot: John F. Kennedy and the Great Space Race* by Douglas Brinkley
- *Remarkably Bright Creatures* by Shelby Van Pelt
- *Zen and the Art of Motorcycle Maintenance: An Inquiry Into Values* by Robert M. Pirsig

### 2025

- *Misery* by Stephen King
- *Four Thousand Weeks: Time Management for Mortals* by Oliver Burkeman
- *The Book of Humans: A Brief History of Culture, Sex, War and the Evolution of Us* by Adam Rutherford — The Book of Humans is a book by Adam Rutherford. It is a book about the history of humans and the evolution of our species.
- *Don't Believe Everything You Think* by Joseph Nguyen — Don't Believe Everything You Think is a book by Joseph Nguyen. It is a book about the power of thought and the importance of critical thinking.
- *The Three-Body Problem* by Cixin Liu — The Three-Body Problem is a science fiction novel by Liu Cixin. It is the first book in the Three-Body Problem trilogy. The novel is set in a future where humanity is threatened by three alien civilizations. The novel is a thought-provoking exploration of the nature of humanity and the universe.
- *John Lewis: A Life* by David Greenberg — John Lewis was a civil rights icon and a true American hero. His life is a testament to the power of nonviolent resistance and the fight for justice. His legacy will continue to inspire generations to come.
- *Holly* by Stephen King
- *Robin* by Dave Itzkoff

### 2024

- *A Little Life* by Hanya Yanagihara
- *Tomorrow, and Tomorrow, and Tomorrow* by Gabrielle Zevin
- *A Gentleman in Moscow* by Amor Towles
- *The Fury* by Alex Michaelides
- *The Wager: A Tale of Shipwreck, Mutiny and Murder* by David Grann
- *Small Mercies* by Dennis Lehane
- *Nineteen Minutes* by Jodi Picoult
- *Harry Potter and the Philosopher's Stone* by J.K. Rowling

### 2023

- *Project Hail Mary* by Andy Weir
- *The Song of Achilles* by Madeline Miller

### 2022

- *Cell* by Stephen King
- *This Is Your Mind on Plants* by Michael Pollan
- *Comedy Comedy Comedy Drama* by Bob Odenkirk
- *1984* by George Orwell
- *The Boys in the Boat: Nine Americans and Their Epic Quest for Gold at the 1936 Berlin Olympics* by Daniel James Brown

### 2021

- *Falling* by T.J. Newman
- *Modern Romance* by Aziz Ansari
- *Northern Spy* by Flynn Berry
- *Hidden Valley Road: Inside the Mind of an American Family* by Robert Kolker
- *Greenlights* by Matthew McConaughey
- *The Bomber Mafia: A Dream, a Temptation, and the Longest Night of the Second World War* by Malcolm Gladwell
- *Anxious People* by Fredrik Backman
- *The Guest List* by Lucy Foley
- *A Promised Land* by Barack Obama
- *The Midnight Library* by Matt Haig
- *The Outsider* by Stephen King
- *Atomic Habits: An Easy & Proven Way to Build Good Habits & Break Bad Ones* by James Clear
- *Recursion* by Blake Crouch
- *Beyond Earth: Our Path to a New Home in the Planets* by Charles Wohlforth

### 2020

- *The Hitchhiker's Guide to the Galaxy* by Douglas Adams
- *The Fourth Estate* by Jeffrey Archer
- *Becoming* by Michelle Obama
- *Educated* by Tara Westover
- *Homo Deus: A History of Tomorrow* by Yuval Noah Harari
- *Nightfall* by Isaac Asimov
- *The Pragmatic Programmer: Your Journey to Mastery, 20th Anniversary Edition* by Andy Hunt
- *The Body: A Guide for Occupants* by Bill Bryson
- *A Short History of Nearly Everything* by Bill Bryson
- *The Silent Patient* by Alex Michaelides

### 2019

- *Eleanor Oliphant Is Completely Fine* by Gail Honeyman
- *Sapiens: A Brief History of Humankind* by Yuval Noah Harari
- *Six Easy Pieces: Essentials of Physics Explained by Its Most Brilliant Teacher* by Richard Feynman
- *Nothing Ventured* by Jeffrey Archer
- *The Grand Design* by Stephen W. Hawking
- *The Dark Bones* by Loreth Anne White
- *Little Fires Everywhere* by Celeste Ng
- *How to Invent Everything: Rebuild All of Civilization (with 96% Fewer Catastrophes This Time)* by Ryan North
- *Fire & Blood* by George R.R. Martin
- *The Help* by Kathryn Stockett

### 2018

- *Origin* by Dan Brown
- *All the Light We Cannot See* by Anthony Doerr
- *Einstein's Dreams* by Alan Lightman
- *Astrophysics for People in a Hurry* by Neil deGrasse Tyson
- *The Eleventh Commandment* by Jeffrey Archer
- *Siddhartha* by Hermann Hesse
- *Benjamin Franklin: An American Life* by Walter Isaacson
- *The Bhagavad Gita* by Krishna-Dwaipayana Vyasa

### 2017

- *Not a Penny More, Not a Penny Less* by Jeffrey Archer
- *Eragon* by Christopher Paolini

### 2016

- *Building Microservices: Designing Fine-Grained Systems* by Sam Newman
- *Making History* by Stephen Fry
- *A Feast for Crows* by George R.R. Martin

### 2015

- *The Golem's Eye* by Jonathan Stroud

### No date recorded

- *Verity* by Colleen Hoover
- *When No One Is Watching* by Alyssa Cole
- *Where the Crawdads Sing* by Delia Owens
- *The Tattooist of Auschwitz* by Heather Morris
- *The Night Sky: An Observer's Guide* by Dennis L. Mammana
- *Superintelligence: Paths, Dangers, Strategies* by Nick Bostrom
- *Deception Point* by Dan Brown
- *Angels & Demons* by Dan Brown
- *Shall We Tell the President?* by Jeffrey Archer
- *Steve Jobs* by Walter Isaacson
- *Ptolemy's Gate* by Jonathan Stroud
- *The Fault in Our Stars* by John Green
- *Digital Fortress* by Dan Brown
- *Surely You're Joking, Mr. Feynman!: Adventures of a Curious Character* by Richard Feynman
- *Why Does the World Exist?: An Existential Detective Story* by Jim Holt
- *How I Braved Anu Aunty & Co-Founded A Million Dollar Company* by Varun Agarwal
- *Paths of Glory* by Jeffrey Archer
- *As the Crow Flies* by Jeffrey Archer
- *The Amulet of Samarkand* by Jonathan Stroud
